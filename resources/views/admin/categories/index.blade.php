...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.categories.create') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded">
        New category
    </a>
</div>
...
<div class="bg-gray-900 p-4 rounded-lg shadow-lg">
    <div class="flex justify-between items-center mb-4">
        ...
        <form method="GET" action="{{ route('admin.categories.index') }}" class="relative w-64">
            ...
            @if(request('search'))
                <a href="{{ route('admin.categories.index') }}" class="absolute right-3 top-1.5 text-orange-400">
        ...
        <table class="min-w-full bg-gray-800 text-white text-sm">
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr>
                    ...
                    <td class="px-4 py-2">
                        @if($category->icon)
                            <img src="{{ asset('storage/' . $category->icon) }}" alt="icon" class="w-6 h-6">
                        @else
                            <span class="text-gray-400 italic">No icon</span>
                        @endif
                    </td>
                    <td class="px-4 py-2">
                        <a href="{{ route('admin.categories.edit', $category->id) }}" class="text-orange-400 hover:underline">
